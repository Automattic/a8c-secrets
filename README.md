...
**Memory hygiene.** Decrypted file contents are held in [`zeroize`](https://docs.rs/zeroize/) buffers where practical so they are cleared when dropped. Private keys use `age`’s `SecretString`.
