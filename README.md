...
- **`edit`** is for interactive use only: stdin and stdout must be a terminal. **Without a file name**, you pick from existing decrypted secrets (run `decrypt` first if none). **With a file name**, you confirm editing or creating that secret before `$EDITOR` runs (trust warning). New secrets are created with `a8c-secrets edit <file>`.
